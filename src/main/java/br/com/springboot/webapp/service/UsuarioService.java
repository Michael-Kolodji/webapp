...
import br.com.springboot.webapp.model.UsuarioModel;
import br.com.springboot.webapp.model.UsuarioSecurityModel;
import br.com.springboot.webapp.repository.GrupoRepository;
import br.com.springboot.webapp.repository.PermissaoRepository;
import br.com.springboot.webapp.repository.UsuarioRepository;
...
public class UsuarioService implements UserDetailsService {
	...
	@Autowired
	private PermissaoRepository permissaRepository;
	...
	public List<GrantedAuthority> buscarPermissoesUsuario(UsuarioEntity usuarioEntity) {
		...
		return this.buscarPermissoesDoGrupo(grupos);
	}

	/***
	 * BUSCA AS PERMISSÕES DO GRUPO
	 */
	private List<GrantedAuthority> buscarPermissoesDoGrupo(List<GrupoEntity> grupos) {

		List<GrantedAuthority> auths = new ArrayList<GrantedAuthority>();

		for (GrupoEntity grupo : grupos) {
			List<PermissaoEntity> lista = permissaRepository.findByGruposIn(grupo);
	...
	public void salvarUsuario(UsuarioModel usuarioModel) {
		...
		for (Long codigoGrupo : usuarioModel.getGrupos()) {
			if (codigoGrupo != null) {
				...
				grupoEntity = grupoRepository.findByCodigo(codigoGrupo);
	...
	public void excluir(Long codigoUsuario){
		UsuarioEntity usuarioEntity = this.usuarioRepository.findByCodigo(codigoUsuario);
	...
	public void alterarUsuario(UsuarioModel usuarioModel){
		...
		UsuarioEntity usuarioEntity =  this.usuarioRepository.findByCodigo(usuarioModel.getCodigo());
